...
pub(crate) fn sanitize_avoids_leaks(entry: &MetaAddr) {
    ...
    assert_eq!(
        sanitized.get_last_success_or_untrusted().timestamp() % TIMESTAMP_TRUNCATION_SECONDS,
        0
    );
    assert_eq!(
        sanitized
    ...
    if let Some(lowest_time) = lowest_time {
        assert!(sanitized.get_last_success_or_untrusted().timestamp() > lowest_time);
    }
    if let Some(highest_time) = highest_time {
        assert!(sanitized.get_last_success_or_untrusted().timestamp() < highest_time);
    }

    // Sanitize to the the gossiped state, even though it's not serialized
    assert_eq!(sanitized.last_connection_state, NeverAttemptedGossiped);
