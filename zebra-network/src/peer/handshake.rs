    ...
    S::Future: Send,
{
    ...
    fn call(&mut self, req: (TcpStream, SocketAddr)) -> Self::Future {
        ...
        let fut = async move {
            ...
            let peer_tx = peer_tx.with(move |msg|
                                       // We need to use future::ready rather than an async fn,
                                       // because we need the sink to be Unpin, and the
                                       // With<Fut, ...> returned by .with is Unpin only if Fut is
                                       // Unpin, and the futures generated by async fns are not
                                       // Unpin. (Also it makes the function simpler.)
                                       future::ready(Ok(outbound_metrics(msg, &addr))));
            ...
            let peer_rx = peer_rx
                .then(move |msg| {
                    inbound_timestamp_and_metrics(
                        msg,
                        addr,
                        remote_services,
                        inbound_ts_collector.clone(),
                    )
                })
                .then(move |msg| {
                    let inv_collector = inv_collector.clone();
...
/// Gather metrics for each outbound message `msg` to peer `remote_addr`.
fn outbound_metrics(msg: Message, remote_addr: &SocketAddr) -> Message {
...
/// Gather message timestamps and metrics for each inbound message `msg` to peer
/// `remote_addr`.
///
/// Sends peer set updates using `inbound_ts_collector`.
async fn inbound_timestamp_and_metrics(
    msg: Result<Message, SerializationError>,
    remote_addr: SocketAddr,
    remote_services: PeerServices,
    mut inbound_ts_collector: mpsc::Sender<MetaAddr>,
    ...
    match &msg {
        Ok(msg) => {
            ...
            let _ = inbound_ts_collector
                .send(MetaAddr::new_responded(&remote_addr, &remote_services))
                .await;
        }
        Err(err) => {
            metrics::counter!(
            ...
            let _ = inbound_ts_collector
                .send(MetaAddr::new_errored(&remote_addr, &remote_services))
                .await;
        }
    }
    msg
...
/// If `result.is_err()`, mark `remote_addr` as failed using `timestamp_collector`.
